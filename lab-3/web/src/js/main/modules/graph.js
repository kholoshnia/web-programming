...
export default class Graph {
   ...
   * @param $xCheckboxes j Query x checkboxes
   * @param $yText jQuery y text
   */
  saveRawValues = ($xCheckboxes, $yText) => {
    if (this.#rawXValue == null || this.#rawYValue == null) return;

    $xCheckboxes.each((index, element) => {
      const $element = $(element);
      if (Number($element.next().text()) === this.#rawXValue) {
        $element.prop('checked', !$element.prop('checked'));
        $element.change();
      }
    });

    $xCheckboxes.map((index, element) => {
